...
// The text input where the user types the Pokémon's name - // pokemonInput
// The button the user clicks to trigger the search - //searchBtn
// The container (div) where we'll show the Pokémon info - //result
...
// - This means: "When tghe button is clicked, run this function."
...
searchBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    result.innerHTML = `<img src="${image}" alt= "${name}"/>
    <h2>${data.name}</h2>
    <p>Type: ${type}</p>
    `;
  } catch (error) {
